...
const $common = {
  ...
  objectFilter (obj, filter) {
    ...
    Object.keys(obj).forEach((key) => {
      let item = obj[key]
      if (filter.call(this, item, key) !== false) {
        target[key] = item
      }
    })
  ...
  objectMap (data, filter) {
    const target = {}
    for (var key in data) {
      let item = data[key]
      let result = filter.call(this, item, key)
      if (result != null) {
  ...
  regs: {
    ...
    password: /(?!^[0-9]*$)(?!^[a-zA-Z]*$)^([a-zA-Z0-9]{6,16})$/,
    picCode: /^\w{4}$/
  },
  ...
  numberFans(num , unit = '.'){
      num = Number(num);
      var yu = num%10000,  chu = num / 10000;
      if( chu <  1){
          if( unit == '.'){
              return num;
          }
          ...
          return this.numberFans( Math.floor(num/10000), unit )
      }
    },
