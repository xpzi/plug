...
const styles = {
  ...
  outerChildStyle: (() => {
    let outerMarginSize = flexible.px2rem(35 * dpr) + 'px'
    let outerWidthSize = flexible.px2rem(70 * dpr) + 'px'
    
    return `
      position: absolute;
      top: 50%;
      left: 50%;
      width: ${outerWidthSize};
      height: ${outerWidthSize};
      margin-top: -${outerMarginSize};
      margin-left: -${outerMarginSize};
      background-color: rgba(0, 0, 0, .7);
      border-radius: ${flexible.px2rem(5 * dpr)}px;
    `
  })(),

  innerChildStyle: (() => {
    let childMarginSize = flexible.px2rem(13 * dpr) + 'px'
    let childWidthSize = flexible.px2rem(26 * dpr) + 'px'
