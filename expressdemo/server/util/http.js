...
var httpUtil = {
    ...
    get: function(url, reqData, callback, encoding, header){
        if({}.toString.call(reqData) == "[object Object]"){
            url += "?"+qs.stringify(reqData);
        }else{
            url += "?"+reqData;
        }
        ...
        this.request(options, "", callback, encoding, header);
    },
    ...
    postJSON: function(url, reqData, callback){
        var client = request.createClient(url);
        client.post('', reqData, function(err, res, body) {
            callback(body);
        });
    }
}
